...
public class ProdukController {
    ...
    public void insertProduct(Produk p) {
        try {
            ...
            if (p.getId_produk() > 0) {
                ...
                this.koneksi.ManipulasiData("UPDATE FITRIARISQINAA_07032.PRODUK_07032 SET NAMA_PRODUK = '" + p.getNama_produk() + "', HARGA ='" + p.getHarga() + "', STOK ='" + p.getStok() + "' WHERE ID_PRODUK =" + p.getId_produk());
            } else {
                System.out.println("INSERT PRODUK");
                this.koneksi.ManipulasiData("INSERT INTO FITRIARISQINAA_07032.PRODUK_07032 (ID_PRODUK, NAMA_PRODUK, HARGA,STOK) VALUES (ID_PRODUK.NEXTVAL, '" + p.getNama_produk() + "', '" + p.getHarga() + "', '" + p.getStok() + "')");
            }
        } catch (Exception e) {
    ...
    public void deleteProduk(int idProduk) {
        try {
            this.koneksi.ManipulasiData("DELETE FITRIARISQINAA_07032.PRODUK_07032 WHERE ID_PRODUK=" + idProduk);
        } catch (Exception e) {
            System.out.println(e);
        }
    }
