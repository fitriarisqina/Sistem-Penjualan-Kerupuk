...
public class TransaksiController {
    ...
    public void insertTransaksi(Transaksi transaksi) {
        try {
            ...
            this.koneksi.ManipulasiData("INSERT INTO FITRIARISQINAA_07032.TRANSAKSI_07032 (ID_TRANSAKSI, ID_USER, JUMLAH_PRODUK, TGL_MASUK, TGL_KELUAR, JML_PRODUK_KEMBALI, TOTAL)VALUES(ID_TRANSAKSI.NEXTVAL, '" + idUser + "','" + jumlahProduk + "', TO_DATE('" + tglMasuk + "','dd/MM/yyyy'), TO_DATE('" + tglKeluar + "','dd/MM/yyyy'), '" + jumlahProdukKembali + "', '" + totalHarga + "')");
            ...
            for (TransaksiDetail transaksiDetail : transaksi.getTransaksiDetails()) {
                this.koneksi.ManipulasiData("INSERT INTO FITRIARISQINAA_07032.TRANSAKSI_DETAIL_07032 (ID_TRANSAKSI_DETAIL, ID_TRANSAKSI, ID_PRODUK, QTY_PRODUK, HARGA_PRODUK, TOTAL_HARGA) VALUES(ID_TRANSAKSI_DETAIL.NEXTVAL, '" + idTransaksi + "', '" + transaksiDetail.getIdProduk() + "', '" + transaksiDetail.getQtyProduk() + "', '" + Double.parseDouble(transaksiDetail.getHargaProduk()) + "', '" + (Double.parseDouble(transaksiDetail.getHargaProduk()) * transaksiDetail.getQtyProduk()) + "')");
                this.koneksi.ManipulasiData("UPDATE FITRIARISQINAA_07032.PRODUK_07032 SET STOK=STOK-" + transaksiDetail.getQtyProduk() + " WHERE ID_PRODUK=" + transaksiDetail.getIdProduk());
                ...
                System.out.println("INSERT DETAIL TOTAL HARGA = " + totalHarga +" | "+transaksiDetail.getIdProduk() );
            }
            this.koneksi.ManipulasiData("UPDATE FITRIARISQINAA_07032.TRANSAKSI_07032 SET JUMLAH_PRODUK='" + jumlahProduk + "',JML_PRODUK_KEMBALI='" + transaksi.getJml_produk_kmbli() + "', TOTAL='" + totalHarga + "' WHERE ID_TRANSAKSI=" + idTransaksi);
    ...
    public ArrayList<Transaksi> getTransaksi(Transaksi transaksi) throws SQLException {
        ...
        ResultSet rs;
        if (transaksi != null) {
            rs = this.koneksi.getData("SELECT * FROM FITRIARISQINAA_07032.TRANSAKSI_07032 JOIN LOGIN_07032 ON LOGIN_07032.ID_USER = TRANSAKSI_07032.ID_USER WHERE TRANSAKSI_07032.ID_TRANSAKSI = " + transaksi.getId_transaksi());
        } else {
            rs = this.koneksi.getData("SELECT * FROM FITRIARISQINAA_07032.TRANSAKSI_07032 JOIN LOGIN_07032 ON LOGIN_07032.ID_USER = TRANSAKSI_07032.ID_USER");
        }
        while (rs.next()) {
